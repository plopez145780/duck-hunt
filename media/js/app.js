/* Varialbe Globales */
var duck = $('#duck') // référence a l'objet duck
...
function deplacerGaucheDroite(hauteurRandDepart, hauteurRandArrive) {
...
function deplacerDroiteGauche(hauteurRandDepart, hauteurRandArrive) {
  //Initialise les propriétés de départ de l'image du canard
  duck.css({
    width: '100px',
    height: '70px',
...
function tourDeJeu() {
  ...
  // Hauteur aléatoire
  let hauteurRandDepart =
    Math.random() * (hauteurFenetre - duck.height() - $('#menu').height())
  let hauteurRandArrive =
    Math.random() * (hauteurFenetre - duck.height() - $('#menu').height())
  vitesseDuck -= pasDeLaVitesse // Décremente la vitesse ( = accélération du canard)
  // random pour le sens de déplacement
  if (Math.random() <= 0.5) {
    deplacerGaucheDroite(hauteurRandDepart, hauteurRandArrive)
  } else {
    deplacerDroiteGauche(hauteurRandDepart, hauteurRandArrive)
  }
}
...
function verifieParametre() {
  ...
  if (!$('#player_name').val()) {
    ...
    $('#erreur')
      .empty()
      .html("Erreur : Le nom n'est pas renseigné")
      .css('color', 'red')
  }
